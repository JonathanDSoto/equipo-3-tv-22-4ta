    ...
    if (isset($_POST['super_token']) && $_POST['super_token'] == $_SESSION['super_token']) {
        switch ($_POST["action"]) {
            case 'create':
                ...
                $brand->createBrand($name, $description, $slug);
                break;
            case 'update':
                $name = strip_tags($_POST['name']);
                $description = strip_tags($_POST['description']);
                $slug = strip_tags(strtr($_POST['name'], " ", "-"));
                $brand_id = strip_tags($_POST['id']);
                $brand = new BrandController();
                $brand->updateBrand($name, $description, $slug,$brand_id);
                break;
            case 'delete':
...
class BrandController
{
    ...
    public function createBrand($name, $description, $slug)
    {
        ...
        if (isset($response->code) &&  $response->code > 0) {
            header("Location:../public/productos.php?success=true");
        } else {
            header("Location:../public/productos.php?error=true");
        }
    }
